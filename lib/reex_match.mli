...
val match_ : ?options:options ->
             int code -> string code -> ?otherwise:'a code -> (t * (int code -> int code -> 'a code)) list -> 'a code
...
val matchk_ : ?options:options ->
              ?otherwise:'a code -> ((t * (int code -> int code -> 'a code)) list ->
             (start:int -> index:int -> len:int -> string -> 'a) code as 'k) -> 'k
